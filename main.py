...
class TitleWindow:
    ...
    def HomeScreen(self):
        ...
        TitleWindow.parent.geometry("800x500")
        Label(text="").pack()
        title = Label(TitleWindow.parent, text="DETECTION OF ANIMAL", bg='yellow', fg='black',
                      font=('arial', 16, "bold"))
        title.pack()
        Label(text="").pack()
        Label(width=80, font=('arial', 10, "bold"), fg='blue',
              text="Farmers  in  India  face  serious  threats  from  pests,  natural calamities  &damage \n by  animals  resulting  in  lower  yields .  Since  safety  of  both human and animal is equally vital.\nSo, animal detection system is necessary in farm areas.").pack()
        Label(text="").pack()
        TitleWindow.loginbtn = Button(TitleWindow.parent, fg='red', text="LOGIN", width=15, height=3,
    ...
    def login(self):
        ...
        self.loginwin.geometry("400x400")
        ...
        self.loginbtn = Button(self.loginwin, width=8, height=2, text="LOGIN", fg='red', font=('arial', 13, "bold"),
    ...
    def MainMenu(self):
        ...
        self.diabtn = Button(self.welwin, width=15, height=2, text="Train Dataset...", font=('arial', 13, "bold"),
                             command=self.openfolderdialog)
        self.diabtn.grid(row=5, column=1, columnspan=2)

        Label(self.welwin, text="").grid(row=6, column=1, columnspan=2)
        self.diabtn = Button(self.welwin, width=15, height=2, text="Test Image", font=('arial', 13, "bold"),
        ...
        self.diabtn = Button(self.welwin, width=15, height=2, text="Predict", font=('arial', 13, "bold"),
                             command=self.predict)
        self.diabtn.grid(row=10, column=1, columnspan=2)

        Label(self.welwin, text="").grid(row=12, column=1, columnspan=2)
        self.logoutbtn = Button(self.welwin, width=10, height=2, text="Logout", font=('arial', 13, "bold"),
                                command=TitleWindow.parent.destroy)
        self.logoutbtn.grid(row=13, column=1, columnspan=2)

        self.welwin.mainloop()

    def openfolderdialog(self):
        TitleWindow.directory = filedialog.askdirectory(parent=TitleWindow.root,
                                                        title='Choose directory with image sequence stack files')
        ...
        (TitleWindow.trainData, TitleWindow.testData, TitleWindow.trainLabels,
         TitleWindow.testLabels) = train_test_split(np.array(TitleWindow.animals), np.array(TitleWindow.labels),
                                                    test_size=0.1,
                                                    random_state=50)
    ...
    def openfiledialog(self):
        TitleWindow.filename = filedialog.askopenfilename(parent=TitleWindow.root, initialdir="/",
